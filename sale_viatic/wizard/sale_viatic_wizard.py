...
class SaleViaticWizardLine(models.TransientModel):
    ...
    product_id = fields.Many2one('product.product', domain=[('viatic_ok', '=', True)], string='Product')
...
class SaleViaticWizard(models.TransientModel):
    ...
    line_ids = fields.One2many('sale.viatic.wizard.line', 'wizard_id', string='Lines', default=_default_lines)
                    ...
                    if line.line_id:
                        ...
                        if line.product_id.id and line.quantity>0:
                            if not open_viatic:
                                ...
                                open_viatic=viatic_obj.create(vals).id

                            line_vals={
                                'sale_viatic_id':open_viatic,
                                'product_id':line.product_id.id,
                                'quantity': line.quantity,
                                'markup':line.markup,
